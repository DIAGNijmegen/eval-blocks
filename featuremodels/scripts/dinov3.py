...
def compute_features(args, model, processor, dataset_type):
    ...
    for idx, batch_data in tqdm(enumerate(dataloader)):
        slice = batch_data["image"].squeeze().cuda()
        img_2d = (slice - slice.min()) / (slice.max() - slice.min())
        img_2d = (img_2d * 255).astype(np.uint8)
        img_rgb = np.stack([img_2d] * 3, axis=-1)
        img_pil = Image.fromarray(img_rgb)

        model_input = processor(images=img_pil, return_tensors="pt")
...
def main(args):
    ...
    print("Loading model...")
    model = AutoModel.from_pretrained("facebook/dinov3-convnext-base-pretrain-lvd1689m")
    processor = AutoImageProcessor.from_pretrained("facebook/dinov3-convnext-base-pretrain-lvd1689m")
...
if __name__ == "__main__":
    description = "DinoV3 Feature Extraction"
